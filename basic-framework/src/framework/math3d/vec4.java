...
public class vec4{
    ...
    public vec4(Object... args){
        init(args);
    }
    
    private void init(Object[] args){
        int ctr=0;
        float[] _M = new float[4];
        for(Object a : args){
            if( a instanceof Number )
                ...
                throw new RuntimeException("Bad type for vec4 constructor");
        }
        
        if( ctr != 4 )
            throw new RuntimeException("Bad number of arguments for vec4 constructor");
    ...
    public vec4 mul(mat4 m){
        ...
        for(int i=0;i<4;++i){
            float total= x*m.get(0,i) + y*m.get(1,i) + z*m.get(2,i) + w*m.get(3,i);
            tmp[i] = total;
        }
        R.x=tmp[0];
        R.y=tmp[1];
    ...
    public vec4 xyzw(){ return new vec4(x, y, z, w); }
