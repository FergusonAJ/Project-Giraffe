...
def main(infile):
    ...
    for L in infp:
        ...
        if L[0].startswith("#"):
            ...
            if WITH_FUR:
                ...
                qn = normaldata[qv.ni]
                rn = normaldata[rv.ni]
                pt = texturedata[pv.ti]
                qt = texturedata[qv.ti]
                rt = texturedata[rv.ti]
                
                v1 = q-p
                v2 = r-p
                A = length(cross(v1,v2))
                n = furdensity * A
                if R.random() <= fract(n):
                    n = int(n)+1
                else:
                ...
                for fi in range(n):
                    ...
                    w2 /= ss
                    p1 = w0*p + w1*q + w2 * r
                    n1 = w0*pn + w1*qn + w2*rn
                    t1 = w0*pt + w1*qt + w2*rt
                    furdata += [p1.x,p1.y,p1.z, n1.x,n1.y,n1.z, t1.x,t1.y, R.random()]
            if DX:
                assert 0
                t=list(reversed(t))
    ...
    if WITH_FUR:
        print(len(furdata)//8,"fur vertices")
    ...
    write_floats( "fur_data", ofp, furdata)
...
if len(sys.argv) == 1:
    ...
    fur_var = tkinter.IntVar()
    with_fur_cb = tkinter.Checkbutton(win,text="Include fur",variable=fur_var)
    with_fur_cb.pack()
    furdensity = 1000
